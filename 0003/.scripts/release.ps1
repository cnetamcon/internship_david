...
$PATH_HTML_MAIN = Join-Path $DIR_BUILD "readme.html" 
...
# Copy page documents
$DIR_VERSIONS_ATTACHMENT = Join-Path $DIR_DOCS attachments
$DIR_VERSIONS_HTML = Join-Path $DIR_DOCS versions_html
$DIR_HOME_SRC = Join-Path $DIR_SCRIPTS "/doc/src/"
$PATH_HOME_PAGE = Join-Path $DIR_SCRIPTS "/doc/home.html"
$DIR_PAGES = Join-Path $DIR_BUILD .docs 

New-Item -Path $DIR_PAGES -ItemType Directory -Force

copy -R "$DIR_VERSIONS_HTML" (Join-Path $DIR_PAGES "versions")
copy -R "$DIR_VERSIONS_ATTACHMENT" $DIR_PAGES
copy -R "$DIR_HOME_SRC" (Join-Path $DIR_PAGES "src")
copy "$PATH_HOME_PAGE" $DIR_PAGES
